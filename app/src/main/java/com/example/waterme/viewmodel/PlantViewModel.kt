...
class PlantViewModel(application: Application): ViewModel() {
    ...
    internal fun scheduleReminder(
        ...
        var data = Data.Builder().putString(WaterReminderWorker.nameKey, plantName)

        // TODO: Generate a OneTimeWorkRequest with the passed in duration, time unit, and data
        //  instance
        val constraints = Constraints.Builder().setTriggerContentUpdateDelay(duration,unit).build()

        val scheduleBuilder = OneTimeWorkRequest
        ...
        WorkManager.getInstance().enqueueUniqueWork(plantName,ExistingWorkPolicy.REPLACE,scheduleBuilder)
    }
}
